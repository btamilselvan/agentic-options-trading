...
Phase 0 (foundation) is done, and Phase 1 is well underway: the screener (component 1, `services/screener/`) and the data collection & quantitative engine (component 2, `services/quant/`) are both implemented end-to-end, on top of a FastAPI skeleton, typed shared contracts, config/secrets scaffolding, an Alembic-managed async DB layer, and a pluggable LLM insight-provider layer (Gemini/Ollama). No state detection, risk, or broker logic is implemented yet — see the phased plan below for what's still ahead.
...
- Schwab's daily-bar price-history endpoint (`get_price_history_every_day`) includes *today's* still-open bar with a `close` that's just today's live price, not a closed value. `SchwabMarketDataProvider.get_candles` explicitly filters it out for `Interval.DAILY` — found live (two unrelated symbols producing bit-for-bit identical `relative_move_vs_*_pct` values was the tell: both had `price == prev_close` exactly, because "yesterday's close" was actually today's own price). If you touch that filter, re-verify prev-day-close against a real quote, not just that tests pass.
...
2. **Data collection & quantitative engine** (implemented — `services/quant/`) — the *canonical* source of all numerical indicators; no other component computes its own. Computed for the current screener candidates each cycle via `MarketDataProvider.get_candles` (same interface the screener uses for quotes, extended with bar history — `static` synthetic or real Schwab price-history): session VWAP, EMA 9/20/50 (+slopes/alignment), RSI, ROC, canonical time-of-day-aligned RVOL, volume acceleration, ATR (+%), key levels (prior day H/L/C, premarket H/L, opening range, session H/L), SPY/QQQ/sector-ETF trend + relative move. Every feature definition is versioned (`feature_definition_version`) and fixture-tested (`tests/test_quant_*.py`). Options Greeks/IV deferred to Phase 2 per §14's own phased plan. Persisted to `ot_feature_snapshots` — no FK to the screener tables (deliberately; correlation is via `correlation_id` only, threaded from each candidate's own).
...
- `services/quant/` — pure, DB-free indicator functions, one module per feature family (`vwap.py`, `moving_averages.py`, `momentum.py`, `volatility.py`, `volume.py`, `levels.py`, `market_context.py` — the last reuses `moving_averages.classify_trend` rather than duplicating trend logic), `engine.py` (`compute_feature_snapshot` — pure assembly into a `FeatureSnapshot`), `service.py` (`compute_snapshots_for_candidates` — the only place that calls `MarketDataProvider.get_candles`, keeping `engine.py` provider-agnostic and independently testable). All America/New_York session-boundary logic lives in `vwap.py`'s `MARKET_TZ`/`SESSION_OPEN`, reused by `volume.py`/`levels.py` rather than redefined.
...
1. Read-only quant pipeline: screener (**done** — `services/screener/`) + quantitative engine (**done** — `services/quant/`, non-options features only) + state detection + activity log. Both against real Schwab data or the synthetic `static` provider. No LLM, no orders yet.
